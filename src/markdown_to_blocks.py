...
def block_to_block(block)-> BlockType:
    ...
    if all(line.strip().split(" ",1)[0][:-1].isdigit() and line.strip().split(" ",1)[0].endswith(".") for line in lines):
...
def text_to_children(text):
    ...
    for text_node in text_nodes:
        html_code = text_node_to_html(text_node)
        children.append(html_code)
    return children
...
def code_to_html_node(block): 
    ...
    text = block[4:-3]
    raw_text_node = TextNode(text, TextNodeType.TEXT)
